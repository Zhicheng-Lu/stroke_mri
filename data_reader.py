...
class DataReader():
	def __init__(self):
		...
		for mode in ['train', 'test']:
			for folder in folders[mode]:
				datasets_path = f'data/segmentation/{folder}/{mode}'
				patient_paths = glob.glob(f'{datasets_path}/*/*/*')

				for patient_path in patient_paths:
					dataset = patient_path.split('/')[4]
					sequence = patient_path.split('/')[5]
					patient = patient_path.split('/')[6]
				
					if not (folder, dataset, patient) in self.segmentation_folders[mode]:
						self.segmentation_folders[mode][(folder, dataset, patient)] = {}
					self.segmentation_folders[mode][(folder, dataset, patient)][sequence] = patient_path
		
			self.segmentation_folders[mode] = [self.segmentation_folders[mode][key] for key in self.segmentation_folders[mode]]
	...
	def read_in_batch_segmentation(self, sequence_dict):
		...
		if 'masks' in sequence_dict:
			mask_paths = glob.glob(f'{sequence_dict["masks"]}/*')
			mask_paths = sorted(mask_paths, key=lambda s: int(re.sub(r'\D', '', s.split('/')[-1]) or 0))
